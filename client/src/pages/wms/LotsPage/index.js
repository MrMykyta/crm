...
import { createWmsLotsColumns } from '../../../components/data/ListPage/columnSchemas/wmsLotsColumns';
...
export default function LotsPage() {
  ...
  const {
    ...
    onActiveViewChange,
    resetGridPrefs,
  } = useGridPrefs('wms.lots');

  const columns = useMemo(
    () => createWmsLotsColumns({ t, locale: i18n.language }),
    [t, i18n.language]
  );
  ...
  if (!canRead) {
    return (
      <div className={s.forbidden}>
        ...
        <p>{t('wms.lots.noReadPermission', 'You do not have permission to view lots.')}</p>
      </div>
    );
  }
  ...
  return (
    <ListPage
      source="wmsLots"
      title={t('wms.lots.title', 'Lots')}
      columns={columns}
      defaultQuery={{ limit: 25, sort: 'createdAt', dir: 'DESC' }}
      emptyStateText={t('wms.lots.empty', 'No lots found')}
          ...
          controls={[
            {
              ...
              render: ({ query, onChange }) => (
                <TextFilter
                  ...
                  onChange={onChange}
                  fieldKey="search"
                  placeholder={t('wms.lots.filters.search', 'Search by lot number...')}
                />
              ),
            },
            {
              ...
              render: ({ query, onChange }) => (
                <TextFilter
                  ...
                  onChange={onChange}
                  fieldKey="productId"
                  placeholder={t('wms.lots.filters.productId', 'Product ID')}
                />
              ),
            },
          ]}
