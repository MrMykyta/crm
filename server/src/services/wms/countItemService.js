...
// countItemService.js (generated)
const { Op } = require('sequelize');
const { CountItem } = require('../../models');
...
module.exports.list = async ({ query = {}, user = {} } = {}) => {
  ...
  const { rows, count } = await CountItem.findAndCountAll({ where,  order, limit, offset });
  return { rows, count, page, limit };
};

module.exports.getById = async (id) => id ? CountItem.findByPk(id, {  }) : null;
module.exports.create  = async (payload = {}) => {
  if (!payload.companyId) throw new Error('companyId is required');
  return CountItem.create(payload);
};
module.exports.update  = async (id, payload = {}) => {
  if (!id) throw new Error('id is required');
  const row = await CountItem.findByPk(id); if (!row) return null;
...
module.exports.remove  = async (id) => id ? CountItem.destroy({ where:{ id } }) : 0;
