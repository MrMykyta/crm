...
const buildWhere = (query = {}, user = {}) => {
  ...
  if (query.q) { where[Op.or] = [{ code: { [Op.iLike]: `%${query.q}%` } }]; }
  return where;
};
...
module.exports.list = async ({ query = {}, user = {} } = {}) => {
  ...
  const { rows, count } = await Location.findAndCountAll({ where, include: [{ model: Warehouse, as:'warehouse' }], order, limit, offset });
  return { rows, count, page, limit };
};

module.exports.getById = async (id) => id ? Location.findByPk(id, { include: [{ model: Warehouse, as:'warehouse' }], }) : null;
...
module.exports.update  = async (id, payload = {}) => {
  ...
  const row = await Location.findByPk(id); if (!row) return null;
