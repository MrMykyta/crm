...
// transferItemService.js (generated)
...
module.exports.list = async ({ query = {}, user = {} } = {}) => {
  ...
  const { rows, count } = await TransferItem.findAndCountAll({ where,  order, limit, offset });
  return { rows, count, page, limit };
};

module.exports.getById = async (id) => id ? TransferItem.findByPk(id, {  }) : null;
module.exports.create  = async (payload = {}) => {
  if (!payload.companyId) throw new Error('companyId is required');
  return TransferItem.create(payload);
};
module.exports.update  = async (id, payload = {}) => {
  if (!id) throw new Error('id is required');
  const row = await TransferItem.findByPk(id); if (!row) return null;
...
module.exports.remove  = async (id) => id ? TransferItem.destroy({ where:{ id } }) : 0;
