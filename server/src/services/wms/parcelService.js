...
module.exports.list = async ({ query = {}, user = {} } = {}) => {
  ...
  const { rows, count } = await Parcel.findAndCountAll({ where,  order, limit, offset });
  return { rows, count, page, limit };
};

module.exports.getById = async (id) => id ? Parcel.findByPk(id, {  }) : null;
...
module.exports.update  = async (id, payload = {}) => {
  ...
  const row = await Parcel.findByPk(id); if (!row) return null;
...
module.exports.remove  = async (id) => id ? Parcel.destroy({ where:{ id } }) : 0;
