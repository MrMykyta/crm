...
// pickWaveService.js (generated)
const { Op } = require('sequelize');
const { PickWave } = require('../../models');
...
module.exports.list = async ({ query = {}, user = {} } = {}) => {
  ...
  const { rows, count } = await PickWave.findAndCountAll({ where,  order, limit, offset });
  return { rows, count, page, limit };
};

module.exports.getById = async (id) => id ? PickWave.findByPk(id, {  }) : null;
module.exports.create  = async (payload = {}) => {
  if (!payload.companyId) throw new Error('companyId is required');
  return PickWave.create(payload);
};
module.exports.update  = async (id, payload = {}) => {
  if (!id) throw new Error('id is required');
  const row = await PickWave.findByPk(id); if (!row) return null;
...
module.exports.remove  = async (id) => id ? PickWave.destroy({ where:{ id } }) : 0;
