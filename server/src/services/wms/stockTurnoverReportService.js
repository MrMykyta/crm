'use strict';

const { QueryTypes } = require('sequelize');
const AppError = require('../../errors/AppError');
const { StockMove } = require('../../models');

const GROUP_BY = new Set(['product', 'warehouse', 'productWarehouse', 'documentType']);

function ensureCompanyId(companyId) {
  if (!companyId) {
    throw new AppError(403, 'Company context required', { code: 'COMPANY_CONTEXT_REQUIRED' });
  }
}

function asText(value) {
  if (value === undefined || value === null) return '';
  return String(value).trim();
}

function normalizeCurrency(value) {
  const text = asText(value).toUpperCase();
  return text || null;
}

function round4(value) {
  return Math.round((Number(value) + Number.EPSILON) * 1e4) / 1e4;
}

function round2(value) {
  return Math.round((Number(value) + Number.EPSILON) * 100) / 100;
}

function validationError(message, details) {
  return new AppError(400, message, { code: 'VALIDATION_ERROR', details });
}

function parseDateBoundary(value, field, isEnd = false) {
  const text = asText(value);
  if (!text) {
    throw validationError(`${field} is required`, { field });
  }

  const dateOnly = /^\d{4}-\d{2}-\d{2}$/.test(text);
  const parsed = new Date(dateOnly ? `${text}T00:00:00.000Z` : text);
  if (Number.isNaN(parsed.getTime())) {
    throw validationError(`${field} is invalid`, { field, value: text });
  }

  if (isEnd) {
    const exclusive = new Date(parsed.getTime());
    exclusive.setTime(exclusive.getTime() + (dateOnly ? 24 * 60 * 60 * 1000 : 1));
    return exclusive;
  }

  return parsed;
}

function normalizeDateRange(query) {
  const dateFrom = parseDateBoundary(query.dateFrom, 'dateFrom');
  const dateToExclusive = parseDateBoundary(query.dateTo, 'dateTo', true);

  if (dateFrom >= dateToExclusive) {
    throw validationError('dateFrom must be before dateTo', {
      dateFrom: query.dateFrom,
      dateTo: query.dateTo,
    });
  }

  return { dateFrom, dateToExclusive };
}

function buildGroup(groupBy) {
  if (groupBy === 'warehouse') {
    return {
      select: `
        tm.warehouse_id AS "warehouseId",
        w.code AS "warehouseCode",
        w.name AS "warehouseName",
        NULL::uuid AS "productId",
        NULL::text AS "productName",
        NULL::text AS "productSku",
        NULL::uuid AS "variantId",
        NULL::text AS "variantName",
        NULL::text AS "variantSku",
        NULL::text AS "documentType",
        tm.currency AS "currency"
      `,
      group: 'tm.warehouse_id, w.code, w.name, tm.currency',
      order: 'w.code ASC NULLS LAST, w.name ASC NULLS LAST, tm.currency ASC',
    };
  }

  if (groupBy === 'productWarehouse') {
    return {
      select: `
        tm.warehouse_id AS "warehouseId",
        w.code AS "warehouseCode",
        w.name AS "warehouseName",
        tm.product_id AS "productId",
        p.name AS "productName",
        p.sku AS "productSku",
        tm.variant_id AS "variantId",
        pv.sku AS "variantName",
        pv.sku AS "variantSku",
        NULL::text AS "documentType",
        tm.currency AS "currency"
      `,
      group: 'tm.warehouse_id, w.code, w.name, tm.product_id, p.name, p.sku, tm.variant_id, pv.sku, tm.currency',
      order: 'p.name ASC NULLS LAST, p.sku ASC NULLS LAST, w.code ASC NULLS LAST, tm.currency ASC',
    };
  }

  if (groupBy === 'documentType') {
    return {
      select: `
        NULL::uuid AS "warehouseId",
        NULL::text AS "warehouseCode",
        NULL::text AS "warehouseName",
        NULL::uuid AS "productId",
        NULL::text AS "productName",
        NULL::text AS "productSku",
        NULL::uuid AS "variantId",
        NULL::text AS "variantName",
        NULL::text AS "variantSku",
        tm.document_type AS "documentType",
        tm.currency AS "currency"
      `,
      group: 'tm.document_type, tm.currency',
      order: 'tm.document_type ASC, tm.currency ASC',
    };
  }

  return {
    select: `
      NULL::uuid AS "warehouseId",
      NULL::text AS "warehouseCode",
      NULL::text AS "warehouseName",
      tm.product_id AS "productId",
      p.name AS "productName",
      p.sku AS "productSku",
      tm.variant_id AS "variantId",
      pv.sku AS "variantName",
      pv.sku AS "variantSku",
      NULL::text AS "documentType",
      tm.currency AS "currency"
    `,
    group: 'tm.product_id, p.name, p.sku, tm.variant_id, pv.sku, tm.currency',
    order: 'p.name ASC NULLS LAST, p.sku ASC NULLS LAST, tm.currency ASC',
  };
}

function buildWhere(filters, replacements) {
  const where = [
    'sm.company_id = :companyId',
    'sm.created_at >= :dateFrom',
    'sm.created_at < :dateToExclusive',
  ];

  if (filters.warehouseId) {
    where.push('sm.warehouse_id = :warehouseId');
    replacements.warehouseId = filters.warehouseId;
  }
  if (filters.productId) {
    where.push('sm.product_id = :productId');
    replacements.productId = filters.productId;
  }
  if (filters.variantId) {
    where.push('sm.variant_id = :variantId');
    replacements.variantId = filters.variantId;
  }
  if (filters.currency) {
    where.push('COALESCE(sm.currency, :currency) = :currency');
    replacements.currency = filters.currency;
  }

  return where.join('\n      AND ');
}

function mapRow(row) {
  const qtyIn = round4(row.qtyIn);
  const qtyOut = round4(row.qtyOut);
  const valueIn = round2(row.valueIn);
  const valueOut = round2(row.valueOut);

  return {
    warehouseId: row.warehouseId || null,
    warehouseCode: row.warehouseCode || null,
    warehouseName: row.warehouseName || null,
    productId: row.productId || null,
    productName: row.productName || null,
    productSku: row.productSku || null,
    variantId: row.variantId || null,
    variantName: row.variantName || null,
    variantSku: row.variantSku || null,
    documentType: row.documentType || null,
    qtyIn,
    qtyOut,
    valueIn,
    valueOut,
    netQty: round4(qtyIn - qtyOut),
    netValue: round2(valueIn - valueOut),
    currency: row.currency || null,
  };
}

async function listStockTurnover(companyId, query = {}, options = {}) {
  ensureCompanyId(companyId);

  const { dateFrom, dateToExclusive } = normalizeDateRange(query || {});
  const groupBy = GROUP_BY.has(query.groupBy) ? query.groupBy : 'product';
  const filters = {
    warehouseId: asText(query.warehouseId) || null,
    productId: asText(query.productId) || null,
    variantId: asText(query.variantId) || null,
    currency: normalizeCurrency(query.currency),
  };

  const replacements = {
    companyId,
    dateFrom,
    dateToExclusive,
  };
  const where = buildWhere(filters, replacements);
  const group = buildGroup(groupBy);

  const rows = await StockMove.sequelize.query(
    `
    WITH typed_moves AS (
      SELECT
        sm.warehouse_id,
        sm.product_id,
        sm.variant_id,
        -- K1.8: expose PZK / WZK as the documentType bucket so groupBy=documentType
        -- separates corrections from their parents.
        CASE
          WHEN sm.ref_type = 'PZ_KOREKTA' THEN 'PZK'
          WHEN sm.ref_type = 'WZ_KOREKTA' THEN 'WZK'
          ELSE sm.ref_type
        END AS document_type,
        COALESCE(sm.currency, :defaultCurrency) AS currency,
        ABS(sm.qty)::numeric AS qty,
        ABS(COALESCE(sm.total_cost, sm.qty * sm.unit_cost, 0))::numeric AS move_value,
        CASE
          WHEN sm.type = 'receipt' AND sm.ref_type = 'PZ' THEN 'in'
          WHEN sm.type = 'adjustment' AND sm.ref_type = 'PW' THEN 'in'
          WHEN sm.type = 'ship' AND sm.ref_type = 'WZ' THEN 'out'
          WHEN sm.type = 'adjustment' AND sm.ref_type = 'RW' THEN 'out'
          WHEN sm.type = 'transfer' AND sm.ref_type = 'MM' AND sm.to_location_id IS NOT NULL THEN 'in'
          WHEN sm.type = 'transfer' AND sm.ref_type = 'MM' AND sm.from_location_id IS NOT NULL THEN 'out'
          -- K1.8: correction reverse moves.
          WHEN sm.type = 'receipt' AND sm.ref_type = 'WZ_KOREKTA' THEN 'in'
          WHEN sm.type = 'ship' AND sm.ref_type = 'PZ_KOREKTA' THEN 'out'
          ELSE NULL
        END AS direction
      FROM stock_moves sm
      WHERE ${where}
    )
    SELECT
      ${group.select},
      SUM(CASE WHEN tm.direction = 'in' THEN tm.qty ELSE 0 END)::numeric AS "qtyIn",
      SUM(CASE WHEN tm.direction = 'out' THEN tm.qty ELSE 0 END)::numeric AS "qtyOut",
      SUM(CASE WHEN tm.direction = 'in' THEN tm.move_value ELSE 0 END)::numeric AS "valueIn",
      SUM(CASE WHEN tm.direction = 'out' THEN tm.move_value ELSE 0 END)::numeric AS "valueOut"
    FROM typed_moves tm
    LEFT JOIN warehouses w
      ON w.id = tm.warehouse_id
     AND w.company_id = :companyId
    LEFT JOIN products p
      ON p.id = tm.product_id
     AND p.company_id = :companyId
    LEFT JOIN product_variants pv
      ON pv.id = tm.variant_id
     AND pv.company_id = :companyId
    WHERE tm.direction IS NOT NULL
    GROUP BY ${group.group}
    ORDER BY ${group.order}
    `,
    {
      replacements: {
        ...replacements,
        defaultCurrency: filters.currency || 'PLN',
      },
      transaction: options.transaction || null,
      type: QueryTypes.SELECT,
    }
  );

  const data = rows.map(mapRow);
  const currencySet = new Set(data.map((row) => row.currency).filter(Boolean));
  const totals = {
    qtyIn: round4(data.reduce((sum, row) => sum + Number(row.qtyIn || 0), 0)),
    qtyOut: round4(data.reduce((sum, row) => sum + Number(row.qtyOut || 0), 0)),
    valueIn: round2(data.reduce((sum, row) => sum + Number(row.valueIn || 0), 0)),
    valueOut: round2(data.reduce((sum, row) => sum + Number(row.valueOut || 0), 0)),
    netQty: round4(data.reduce((sum, row) => sum + Number(row.netQty || 0), 0)),
    netValue: round2(data.reduce((sum, row) => sum + Number(row.netValue || 0), 0)),
    currency: filters.currency || (currencySet.size === 1 ? [...currencySet][0] : (currencySet.size > 1 ? 'MIXED' : null)),
  };

  return { data, totals };
}

module.exports = {
  listStockTurnover,
};
