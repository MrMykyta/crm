...
// shipmentItemService.js (generated)
const { Op } = require('sequelize');
const { ShipmentItem } = require('../../models');
...
module.exports.list = async ({ query = {}, user = {} } = {}) => {
  ...
  const { rows, count } = await ShipmentItem.findAndCountAll({ where,  order, limit, offset });
  return { rows, count, page, limit };
};

module.exports.getById = async (id) => id ? ShipmentItem.findByPk(id, {  }) : null;
module.exports.create  = async (payload = {}) => {
  if (!payload.companyId) throw new Error('companyId is required');
  return ShipmentItem.create(payload);
};
module.exports.update  = async (id, payload = {}) => {
  if (!id) throw new Error('id is required');
  const row = await ShipmentItem.findByPk(id); if (!row) return null;
...
module.exports.remove  = async (id) => id ? ShipmentItem.destroy({ where:{ id } }) : 0;
